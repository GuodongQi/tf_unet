...
def conv_block(inputs, filters_unm, kernels, strides, use_bias):
    ...
    if not use_bias:
        ...
        x = tf.layers.conv2d(inputs=inputs, filters=filters_unm, kernel_size=kernels, strides=strides, padding="same",
                             use_bias=use_bias, bias_initializer=tf.truncated_normal_initializer())
...
def u_net(input_data):
    ...
    with tf.name_scope("head"):
        x = conv_block(up1, filters_unm=64, kernels=(3, 3), strides=(2, 2), use_bias=True)  # 216*64
        ...
        x = conv_block(x, filters_unm=512, kernels=(3, 3), strides=(2, 2), use_bias=False)  # 32 * 8 * 512

    with tf.name_scope("fc"):
        x = tf.layers.conv2d(x, filters=1, kernel_size=(13, 8), strides=(1, 1), padding='valid',
                             use_bias=True)  # batch*20 * 1 * 1
        x = x[..., 0, 0]
        return x
...
def eva_loss(label, predict):
    """eval loss"""
    loss = tf.losses.softmax_cross_entropy(label, predict)
    return loss
